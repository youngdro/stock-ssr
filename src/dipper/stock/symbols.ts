...
const addPrefix = (code) => {
    ...
    return firstChar == "6" ? dataSource.prefix.sh + code : dataSource.prefix.sz + code;
};
...
const symbols = {
    getStockList: async () => {
        let result = [];
        for (let i = 1; i < 40; i++) {
            let list_str = await digger(format(dataSource.stock_list.url, i));
            let lo = eval(<string>list_str);
            if (!lo) break;
    ...
    getStockIssue: async (code) => {
        ...
        return await digger(format(dataSource.stock_issue.url, code), options);
    },
    /**
     * @returns {json} 行业板块列表
     */
    getIndustryList: async () => {
        let list_str = <string>await digger(dataSource.stock_board.url);
    ...
    getBoards: async (code) => {
        ...
        let list_str = <string>await digger(format(dataSource.stock_sboard.url, market, code));
