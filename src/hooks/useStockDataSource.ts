...
export const useGetLatestDailyHis = (code: string, options?: Options<IKLineItem[], string[]>) => {
  const { data = [], loading } = useRequest(() => getLatestDailyHis({ params: { code } }), options);
  return { data, loading };
};

export const useGetLatestWeekHis = (code: string, options?: Options<IKLineItem[], string[]>) => {
  const { data = [], loading } = useRequest(() => getLatestWeekHis({ params: { code } }), options);
  return { data, loading };
};

export const useGetLatestMonthHis = (code: string, options?: Options<IKLineItem[], string[]>) => {
  const { data = [], loading } = useRequest(() => getLatestMonthHis({ params: { code } }), options);
  return { data, loading };
};
