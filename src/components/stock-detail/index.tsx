...
const StockDetail: React.FC<StockDetailProps> = ({ code }) => {
  ...
  const { data: latestWeekHis, loading: weekLoading } = useGetLatestWeekHis(code, {
    ready: !!tabKeyVisitedMap['w'],
  });
  const { data: latestMonthHis, loading: monthLoading } = useGetLatestMonthHis(code, {
  ...
  const items: TabsProps['items'] = [
    {
      ...
      label: `日K线`,
      forceRender: true,
      children: <Spin spinning={dailyLoading}>{renderKLineChart(latestDailyHis)}</Spin>,
    },
    {
      key: 'w',
      label: `周K线`,
      forceRender: true,
      children: <Spin spinning={weekLoading}>{renderKLineChart(latestWeekHis)}</Spin>,
    },
    {
      key: 'm',
      label: `月K线`,
      forceRender: true,
      children: <Spin spinning={monthLoading}>{renderKLineChart(latestMonthHis)}</Spin>,
    },
  ];
